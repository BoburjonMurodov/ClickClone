...
class SignInVerifyScreen(private val phone: String) : Screen {
    ...
    override fun Content() {
        val viewModel: SignInVerifyScreenContract.ViewModel = hiltScreenModel()
...
private fun VerifyScreenContent(
    phone: String,
    uiState: State<SignInVerifyScreenContract.UIState>,
    onEventDispatcher: (SignInVerifyScreenContract.Intent) -> Unit
) {
        ...
        Box(Modifier.fillMaxWidth()) {
            Icon(
                modifier = Modifier
                    .height(24.dp)
                    .width(24.dp)
                    .clickable { onEventDispatcher(SignInVerifyScreenContract.Intent.CLickBack) },
...
//                if (it.length == 6) onEventDispatcher(SignUpVerifyScreenContract.Intent.ClickButton(code))
        ...
        AppButton(
            text = "Davom etish",
            onClick = { onEventDispatcher(SignInVerifyScreenContract.Intent.ClickButton(code)) },
...
private fun VerifyScreenPreview() {
    val uiState = remember {
        mutableStateOf(SignInVerifyScreenContract.UIState())
    }
